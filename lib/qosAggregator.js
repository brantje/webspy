...
QosAggregator.prototype.createTagStatFromPingAggregationDocument = function(document, callback) {
  ...
  var intervalBuilder = new IntervalBuilder();
  var owner = document._id.owner;
  Check.findForTag(document._id.owner, document._id.tag, function(err, checks) {
    if (err) return callback(err);
    checks.forEach(function(check) {
      intervalBuilder.addTarget(check);
    });
    intervalBuilder.build(document.start, document.end, function(err2, outages, downtime, testedTime,document) {
...
QosAggregator.prototype.updateLastHourQos = function(callback) {
  var now = new Date(Date.now() - 1000 * 60 * 2); // 6 minutes in the past, to accommodate script running every 5 minutes
...
QosAggregator.prototype.updateLastDayQos = function(callback) {
  var now = new Date(Date.now() - 1000 * 60 * 2); // 1 hour and 6 minutes in the past, to accommodate script running every hour
