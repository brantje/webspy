...
function Monitor(config) {
  ...
  this.ssl = url.parse(config.apiUrl).protocol.indexOf('https') > -1;
...
Monitor.prototype.findChecksNeedingPoll = function(callback) {
  ...
  options.headers = {
    'WebSpy_Monitor': this.config.name
  };
...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  var pollerCallback = function(err, time, res, pollerDetails) {
    if (err) {
      if(!check.isRechecked) {
        check.isRechecked = true;
        setTimeout(function () {
          self.pollCheck(check, function (err) {
            if (err) console.error(err);
          });
  ...
  try {
    var timeout = check.timeout || this.config.timeout;
    p = new Poller(check.url, timeout, pollerCallback,check);
...
Monitor.prototype.createPing = function(error, check, timestamp, time, details, callback) {
  ...
  var options = url.parse(this.config.apiUrl + '/pings?&apikey='+check.owner.apiKeys[0].apiKey);
  options.method = 'POST';
  options.headers = {
